...
public class JAXRSClientInterpreter implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method meth, Object[] args) throws Throwable {
        ...
        if ("toString".equals(meth.getName())) {
            return "ESS JAXRS Proxy for: " + serviceInterface.getName();
        }
        ...
        // Add only further path segments, if they are not parameters
        if (meth.isAnnotationPresent(Path.class) && !meth.getAnnotation(Path.class).value().contains("{")) {
        ...
        if (args != null && args.length > 0) {
            if (meth.getParameterAnnotations()[0].length > 0 && meth.getParameterAnnotations()[0][0].annotationType() == PathParam.class) {
                ...
                // We assume, that if the first pathParam is set, it is the id of a touchpoint and add it to the requestUrl
                requestUrl += "/" + args[0];

                // If there is a second arg, we assign it to the body
                if (args.length > 1) {
        ...
        if (requestBodyData != null) {
            ...
            jsonSerialiser.writeObject(requestBodyData, outputStream);

            // TODO: create an ByteArrayEntity from the stream's content, assiging it to requestBodyDataAsJson
            requestBodyDataAsJson = new ByteArrayEntity(outputStream.toByteArray());
            ...
            if (meth.isAnnotationPresent(POST.class)) {
               ((HttpPost)request).setEntity(requestBodyDataAsJson);
            } else if (meth.isAnnotationPresent(PUT.class)) {
               ((HttpPut)request).setEntity(requestBodyDataAsJson);
            }
